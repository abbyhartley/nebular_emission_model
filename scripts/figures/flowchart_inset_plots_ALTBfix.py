...
FLUX_SCALE = 1e-17
SEED = 0
N_MC = 20          # MC mean prediction
N_PLOT = 50_000   # downsample for faster plotting (hexbin is fine with this)
...
def square_hexbin(ax, x, y, *, cmap=CMAP, mincnt=MINCNT, gridsize=70):
    ...
    _xy=np.concatenate([x,y]); _xy=_xy[np.isfinite(_xy)]; _q1,_q3=np.percentile(_xy,[25,75]); _fe=3.0*(_q3-_q1); _kp=_xy[(_xy>=_q1-_fe)&(_xy<=_q3+_fe)]; lo=float(_kp.min())
    hi=float(_kp.max())
    pad = 0.02 * (hi - lo)
    lo -= pad; hi += pad
