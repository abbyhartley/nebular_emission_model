# in_survey_lums_fig_pretty.py
#
# In-survey analogue of cross_survey_lums_fig1_pretty.py.
# Instead of testing each flow on the OTHER survey, this tests each flow on the
# SAME survey it was trained on:
#   Top row:    SDSS -> SDSS  (SDSS-trained flow sampled on SDSS training data)
#   Bottom row: DESI -> DESI  (DESI-trained flow sampled on DESI training data)
#
# Everything else (axis-limit logic, hexbin style, 1:1 line, per-line stats box,
# cmasher bubblegum colormap, fonts) is IDENTICAL to the cross-survey figure so
# the two can be compared side by side to see how the in-survey scatter differs
# from the cross-survey transfer scatter.
...
FLOW_SDSS = Path("/oak/stanford/groups/cyaolai/AbbyHartley/gfc_NFs/nebular_emission_model/models/nf_sdss_ALTB.eqx")
META_SDSS = Path("/oak/stanford/groups/cyaolai/AbbyHartley/gfc_NFs/nebular_emission_model/models/nf_sdss_ALTB_meta.pkl")

FLOW_DESI = Path("/oak/stanford/groups/cyaolai/AbbyHartley/gfc_NFs/nebular_emission_model/models/nf_desi_ALTB.eqx")
META_DESI = Path("/oak/stanford/groups/cyaolai/AbbyHartley/gfc_NFs/nebular_emission_model/models/nf_desi_ALTB_meta.pkl")
...
# inverted bubblegum: white (low density) -> bubblegum's DARK navy end (high density), white background
from matplotlib.colors import LinearSegmentedColormap as _LSC
CMAP = _LSC.from_list("bubblegum_dense",
                      ["white", cmr.bubblegum(0.85), cmr.bubblegum(0.6),
...
def hex_panel(ax, x, y, *, lo, hi, mincnt=5):
    ...
    ax.plot([lo, hi], [lo, hi], color="black", lw=3.0, ls=":", alpha=0.95)  # thicker dotted 1:1
    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
...
def main():
    ...
    # IN-SURVEY: each flow sampled on the survey it was trained on.
    ratios_sdss_on_sdss = sample_ratios(flow_sdss, meta_sdss, df_sdss, seed=SEED + 1, n_mc=N_MC)
    ratios_desi_on_desi = sample_ratios(flow_desi, meta_desi, df_desi, seed=SEED + 2, n_mc=N_MC)
    ...
    for (label, desi_col, sdss_col, substr_sdss, substr_desi) in PLOT_LINES:
        # Top (SDSS -> SDSS)
        yobs_top = true_logL(df_sdss, "sdss", desi_col, sdss_col)
        ypred_top = pred_logL_oii_total(df_sdss, meta_sdss, ratios_sdss_on_sdss) if desi_col == "OII_TOTAL" else \
                    pred_logL_single(df_sdss, meta_sdss, ratios_sdss_on_sdss, substr_sdss)

        # Bottom (DESI -> DESI)
        yobs_bot = true_logL(df_desi, "desi", desi_col, sdss_col)
        ypred_bot = pred_logL_oii_total(df_desi, meta_desi, ratios_desi_on_desi) if desi_col == "OII_TOTAL" else \
                    pred_logL_single(df_desi, meta_desi, ratios_desi_on_desi, substr_desi)
    ...
    for j, (label, desi_col, sdss_col, substr_sdss, substr_desi) in enumerate(PLOT_LINES):
        ...
        # Top: DESI -> DESI  (row predicts DESI galaxies, matching cross-survey top)
        yobs = true_logL(df_desi, "desi", desi_col, sdss_col)
        ypred = pred_logL_oii_total(df_desi, meta_desi, ratios_desi_on_desi) if desi_col == "OII_TOTAL" else \
                pred_logL_single(df_desi, meta_desi, ratios_desi_on_desi, substr_desi)
        ...
        # Bottom: SDSS -> SDSS  (row predicts SDSS galaxies, matching cross-survey bottom)
        yobs2 = true_logL(df_sdss, "sdss", desi_col, sdss_col)
        ypred2 = pred_logL_oii_total(df_sdss, meta_sdss, ratios_sdss_on_sdss) if desi_col == "OII_TOTAL" else \
                 pred_logL_single(df_sdss, meta_sdss, ratios_sdss_on_sdss, substr_sdss)
        ...
        if j == 0:
            ...
            axes[1, j].tick_params(labelleft=True)

    fig.text(-0.02, 0.74, r"DESI$\rightarrow$DESI", rotation=90, va="center", ha="left", fontsize=15)
    fig.text(-0.02, 0.28, r"SDSS$\rightarrow$SDSS", rotation=90, va="center", ha="left", fontsize=15)
