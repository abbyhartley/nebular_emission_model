...
n_H fixed at the FSPS/Byler default (100 cm^-3). Saves docs/byler_grid_dense.csv.
"""
import os
...
LOGZ = np.round(np.arange(-2.0, 0.51, 0.1), 3)      # gas = stellar
LOGU = np.round(np.arange(-4.0, -0.99, 0.1), 3)
...
df.to_csv(REPO + "docs/byler_grid_dense.csv", index=False)
...
print("Wrote docs/byler_grid_dense.csv")
