...
# Using an OII_TOTAL virtual line built from (3726+3729) in both true and predicted space.
PLOT_LINES = [
    ("H$\\beta$",   "HBETA_FLUX",      "H_BETA_FLUX",      "H_BETA",    "HBETA"),
    ("[NII]6584",   "NII_6584_FLUX",   "NII_6584_FLUX",    "NII_6584",  "NII_6584"),
    ("[OII]3727",   "OII_TOTAL",       "OII_TOTAL",        "OII_TOTAL", "OII_TOTAL"),  # handled specially
...
def prep_df(fits_path, survey):
    ...
    if survey == "sdss":
        ...
        # required measured lines for this plot:
        required_sdss = ["H_BETA_FLUX", "NII_6584_FLUX", "OIII_5007_FLUX", "OII_3726_FLUX", "OII_3729_FLUX"]
        for c in required_sdss:
        ...
        required_desi = ["HBETA_FLUX", "NII_6584_FLUX", "OIII_5007_FLUX", "OII_3726_FLUX", "OII_3729_FLUX"]
        for c in required_desi:
            x = df[c].to_numpy(float)
            mask &= np.isfinite(x) & (x > 0)
...
def pred_logL_oii_total(df, meta, ratios8):
    """
    log L(OII_total) = logLHa + log10(10^r3726 + 10^r3729),
    where rXXXX = log(F_OIIXXXX / F_Ha)
    """
    out_cols = meta["resolved"]["out_cols"]
...
def true_logL(df, survey, desi_col, sdss_col):
    if desi_col == "OII_TOTAL" and sdss_col == "OII_TOTAL":
        if survey == "desi":
...
def main():
    ...
    for i, (label, desi_col, sdss_col, substr_sdss, substr_desi) in enumerate(PLOT_LINES):
        ...
        if desi_col == "OII_TOTAL":
            ypred = pred_logL_oii_total(df_desi, meta_sdss, ratios_sdss_on_desi)
        else:
            ypred = pred_logL_single(df_desi, meta_sdss, ratios_sdss_on_desi, substr_sdss)
        ...
        if desi_col == "OII_TOTAL":
            ypred2 = pred_logL_oii_total(df_sdss, meta_desi, ratios_desi_on_sdss)
        else:
            ypred2 = pred_logL_single(df_sdss, meta_desi, ratios_desi_on_sdss, substr_desi)
    ...
    out = "cross_survey_transfer_panels_noSII_noHgamma_OIItotal.png"
