# train_alt_sdss.py <infile.fits> <out_flow.eqx> <out_meta.pkl>
# Parametrized copy of train_nf_sdss.py (identical config/logic) for the ALT-B SDSS sample.
import sys
...
z_col = "Z_1"; logm_col = "LOGM_COLOR"; FLUX_SCALE = 1e-17; ha_flux_col = "H_ALPHA_FLUX"
line_flux_cols = ["H_BETA_FLUX", "H_GAMMA_FLUX", "NII_6584_FLUX", "SII_6717_FLUX", "SII_6731_FLUX",
...
def main():
    ...
    m = np.isfinite(df[z_col].astype(float)) & np.isfinite(df[logm_col].astype(float))
    m &= np.isfinite(df["_HA_FLUX_CGS"]) & (df["_HA_FLUX_CGS"] > 0)
    for c in line_flux_cols:
        m &= np.isfinite(df[f"_{c}_CGS"]) & (df[f"_{c}_CGS"] > 0)
    df = df.loc[m].copy().reset_index(drop=True)
    ...
    line_aliases_log = {
        "hbeta": ["LOG10_H_BETA_FLUX"], "hgamma": ["LOG10_H_GAMMA_FLUX"], "nii6584": ["LOG10_NII_6584_FLUX"],
        "sii6716": ["LOG10_SII_6717_FLUX"], "sii6731": ["LOG10_SII_6731_FLUX"],
    ...
    meta["extra"] = dict(infile=str(infile), n_train=len(res["df_train"]), note="ALT-B SDSS flow")
