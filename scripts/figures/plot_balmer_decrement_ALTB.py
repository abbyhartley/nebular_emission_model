...
def main():
    ...
    for ax, (cond, k_obs, k_in, k_cross, l_obs, l_in, l_cross, c_obs, c_in, c_cross) in zip(axes, panels):
        ...
        ax.legend(frameon=True, framealpha=0.9, loc="upper right", fontsize=10, labelspacing=0.3, handlelength=1.4,
                  title="Fraction below floor:", title_fontsize=10)
    axes[0].set_ylabel("Normalized density")
    for ext in ("png", "pdf"):
        Path(REPO + "figs").mkdir(exist_ok=True)
        out = REPO + f"figs_ALTB/balmer_decrement_dist.{ext}"
