...
def main():
    ...
    tab.to_csv(REPO / "docs" / "balmer_conditioning_results_genuineS5.csv", index=False)
    print("\n" + tab.to_string(index=False), flush=True)
    print("\nWrote docs/balmer_conditioning_results_genuineS5.csv", flush=True)
    ...
    fig.tight_layout()
    for e in ("png", "pdf"):
        fig.savefig(REPO / "figs" / f"balmer_conditioning_scatter_genuineS5.{e}", dpi=180, bbox_inches="tight")
