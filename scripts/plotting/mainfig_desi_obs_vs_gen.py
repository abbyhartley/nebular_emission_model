...
LINE_SPECS = [
    ...
    ("[SII]6717",    "SII_6716_FLUX"),   # SDSS has 6717
...
def prepare_desi_df(fits_path):
    ...
    for _, col in LINE_SPECS:
        if col not in df.columns:
            raise KeyError(f"Missing {col} in DESI file.")
...
# Plotting
# -------------------------
def plot_obs_vs_gen_grid(y_true, y_pred, out_png="fig1_desi_obs_vs_gen.png"):
...
def plot_ratio_plane(df, y_pred, out_png="fig1b_desi_ratio_plane.png"):
    """
    ...
    idx_hb = [c for _, c in LINE_SPECS].index("HBETA_FLUX")
    ...
    axes[0].set_title("Observed (DESI)")
...
def main():
    ...
    plot_obs_vs_gen_grid(y_true, y_pred, out_png="fig1_desi_obs_vs_gen.png")
    # optional companion ratio plot:
    plot_ratio_plane(df, y_pred, out_png="fig1b_desi_ratio_plane.png")
