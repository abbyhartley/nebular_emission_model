...
def main():
    ...
    desi_fits = Path("/oak/stanford/groups/cyaolai/AbbyHartley/gfc_NFs/desi_bgs_sdss_like_rlt17p77_with_LOGM_COLOR.fits")

    N_MC = 50
    res_sdss = eval_one(flow, meta, sdss_fits, "sdss", seed=0, n_mc=N_MC)
    res_desi = eval_one(flow, meta, desi_fits, "desi", seed=1, n_mc=N_MC)

    out_csv = Path("/oak/stanford/groups/cyaolai/AbbyHartley/gfc_NFs/nebular_emission_model/nf_eval_sdss_metrics_vs_sdsslike_desi.csv")
    pd.concat(
