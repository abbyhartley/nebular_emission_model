# defend_selection_cont3.py
#
# Selection with the paper cuts PLUS a RELAXED continuum-S/N cut (>3 instead of >5):
#   (1) type == GALAXY (DESI: main/bright + ZWARN==0)
#   (2) z > 0.05, ZWARN == 0
#   (3) continuum S/N > 3   (DESI SNR_R ; SDSS SN_MEDIAN)   [relaxed from 5]
#   (4) per-line S/N > 5 on ALL 9 lines incl. Halpha
# Decompose exclusions by which cut removes each galaxy; validate counts printed.
...
Z_MIN, CONT_MIN, LINE_MIN = 0.05, 3.0, 5.0
...
def pack(rec, flux_cols, kind, z, zwarn, logm, cont, base_extra=True):
    ...
    return dict(min9=snr9.min(1).astype(np.float32), arg9=snr9.argmin(1).astype(np.int8),
...
def masks(a):
    base = a["base"]
    pz = a["z"] > Z_MIN
    pc = a["cont"] > CONT_MIN      # relaxed continuum cut
    pl = a["min9"] > LINE_MIN
    sel = base & pz & pc & pl
    ex_z = base & ~pz
    ex_c = base & pz & ~pc
...
def main():
    ...
    for tag, A in DATA.items():
        m = masks(A); M[tag] = m
        print(f"[{tag}] base={m['base'].sum():,}  selected(cont S/N>3)={m['sel'].sum():,}", flush=True)
        print(f"       excl z<0.05={m['ex_z'].sum():,}  excl continuum(<=3)={m['ex_c'].sum():,}  "
              f"excl per-line={m['ex_l'].sum():,}", flush=True)
    ...
    # ---------- FIGURE 1: BPT colored by continuum S/N, weakest-line S/N, redshift ----------
    fig, ax = plt.subplots(2, 3, figsize=(16.5, 11), constrained_layout=True)
    nrm_c = TwoSlopeNorm(vcenter=np.log10(CONT_MIN), vmin=np.log10(0.5), vmax=np.log10(60.0))
    nrm_l = TwoSlopeNorm(vcenter=np.log10(LINE_MIN), vmin=np.log10(1.0), vmax=np.log10(60.0))
    for r, tag in enumerate(["SDSS", "DESI"]):
        A, m = DATA[tag], M[tag]; b = m["base"]
        h0 = bpt_col(ax[r, 0], A["x"][b], A["y"][b], np.log10(np.clip(A["cont"][b], 1e-2, None)), CMAP_SNR, nrm_c)
        contour(ax[r, 0], A["x"][m["sel"]], A["y"][m["sel"]])
        c0 = fig.colorbar(h0, ax=ax[r, 0], shrink=0.85); c0.set_label(r"med. $\log_{10}$(continuum S/N)")
        c0.ax.axhline(np.log10(CONT_MIN), color="k", lw=2)
        h1 = bpt_col(ax[r, 1], A["x"][b], A["y"][b], np.log10(np.clip(A["min9"][b], 1e-2, None)), CMAP_SNR, nrm_l)
        ...
        c1.ax.axhline(np.log10(LINE_MIN), color="k", lw=2)
    ...
    ax[0, 0].set_title("continuum S/N (cut at 3)"); ax[0, 1].set_title("weakest-line S/N (cut at 5)")
    ax[0, 2].set_title("redshift")
    ax[0, 0].plot([], [], "k-", lw=1.7, label="selected 68/95%"); ax[0, 0].legend(loc="lower left")
    fig.suptitle("Selection with relaxed continuum S/N>3 (+ per-line S/N>5 on all 9, z>0.05); selected overlaid",
                 fontsize=15)
    fig.savefig("bpt_selection_cont3.png", dpi=210, bbox_inches="tight")
    print("Saved: bpt_selection_cont3.png", flush=True)
    ...
    for r, tag in enumerate(["SDSS", "DESI"]):
        ...
        ax0.hist(np.clip(A["cont"][m["ex_c"]], 0.3, 300), bins=sb, density=True, histtype="step", lw=2,
                 color=C_CONT, label=f"excl: continuum<=3 ({m['ex_c'].sum():,})")
        ax0.axvline(CONT_MIN, color="k", ls=":", lw=1.6)
        ax0.set_xscale("log"); ax0.set_xlabel("continuum S/N"); ax0.set_ylabel(f"{tag}\nnormalized density"); ax0.legend()
        ax1 = a2[r, 1]
        ...
        ax1.axvline(LINE_MIN, color="k", ls=":", lw=1.6); ax1.set_xscale("log")
    ...
    a2[0, 0].set_title("continuum S/N (cut at 3)"); a2[0, 1].set_title("weakest-line S/N (cut at 5)")
    a2[0, 2].set_title("redshift"); a2[0, 3].set_title("stellar mass")
    fig2.suptitle("Relaxed continuum S/N>3 (+ per-line S/N>5 on all 9, z>0.05): what each cut removes", fontsize=14)
    fig2.savefig("selection_excluded_cont3.png", dpi=210, bbox_inches="tight")
    print("Saved: selection_excluded_cont3.png", flush=True)
