"""
Figure 1 (redesigned flow chart) v3.

v3 tweaks (author feedback):
  - card background #FFDFD9
  - tighter columns 2 & 3 (narrower cards, in/cross insets pulled together)
  - "4. Joint structure" header sits in the white gap between the stacked cards
  - inter-panel arrows + labels kept entirely within the white gaps
  - bold/larger arrow labels, larger text; box width == the plots it sits between,
    centered; box->learned arrow clear of the "learned joint" label; nothing OOB;
    validation split into two stacked cards; no "u" arrow.
"""
...
OUT = REPO + "figs/fig1_flowchart_assembled_v3"
...
def main():
    ...
    cards = {
        "P1": (0.020, 0.130, 0.300, 0.745, "1.  Inputs"),
        "P2": (0.395, 0.130, 0.155, 0.745, "2.  Conditional model"),
        "P3": (0.625, 0.535, 0.340, 0.340, "3.  Predictive accuracy"),
        "P4": (0.625, 0.130, 0.340, 0.340, "4.  Joint structure"),
    }
    ...
    # ================= Panel 1: Inputs =================
    ax_in = fig.add_axes(sqrect(c1, 0.470, 0.360))
    ...
    fig.text(c1, 0.355, r"Targets  $X=\{\log_{10}(F_{\rm line}/F_{H\alpha})\}$", ha="center", va="top", fontsize=16)
    fig.text(c1, 0.303,
    ...
    base_h = 0.185
    box_w = SQ * base_h
    ax_base = fig.add_axes(sqrect(c2, 0.640, base_h)); ax_base.axis("off")
    ...
    ax_learn = fig.add_axes(sqrect(c2, 0.235, base_h))
    filled_density(ax_learn, nii_ha, oiii_hb, [-1.5, 0.0], [-1.0, 1.1])
    ax_learn.set_xlabel(r"$\log$([N II]/H$\alpha$)", fontsize=13)
    ax_learn.set_ylabel(r"$\log$([O III]/H$\beta$)", fontsize=13)
    ax_learn.tick_params(labelsize=10)
    fig.text(c2, 0.175, r"learned joint  $p(X\,|\,u)$", ha="center", va="top", fontsize=14)
    bh = 0.115
    bx0, by0 = c2 - box_w / 2, 0.455
    fig.add_artist(FancyBboxPatch((bx0, by0), box_w, bh, boxstyle="round,pad=0.004,rounding_size=0.012",
                   transform=fig.transFigure, facecolor="white", edgecolor=ACCENT, lw=2.4, zorder=3))
    fig.text(c2, by0 + bh / 2, "Conditional\nnormalizing flow\n" + r"$f(X;u)$",
             ha="center", va="center", fontsize=12, color=ACCENT, zorder=4)
    fig.add_artist(FancyArrowPatch((c2, 0.635), (c2, by0 + bh + 0.004),
                   transform=fig.transFigure, arrowstyle="-|>", mutation_scale=22, lw=2.6, color=ACCENT, zorder=2))
    fig.add_artist(FancyArrowPatch((c2, by0 - 0.004), (c2, 0.421),
                   transform=fig.transFigure, arrowstyle="-|>", mutation_scale=22, lw=2.6, color=ACCENT, zorder=2))

    # ================= Panel 3: Predictive accuracy =================
    ins_h = 0.235
    ax_c = fig.add_axes(sqrect(0.729, 0.600, ins_h)); ax_d = fig.add_axes(sqrect(0.861, 0.600, ins_h))
    ...
    for a, ttl, rho in [(ax_c, "In-survey (DESI)", rho_dd), (ax_d, r"Cross-survey (DESI$\to$SDSS)", rho_ds)]:
        ...
        a.text(0.06, 0.93, rf"$\rho={rho:.2f}$", transform=a.transAxes, fontsize=14, va="top",
               bbox=dict(boxstyle="round", fc="white", ec="0.7", alpha=0.9))
    ax_c.set_ylabel(r"$\log L_{\rm[OIII],\ pred}$", fontsize=13)

    # ================= Panel 4: Joint structure =================
    n = len(LABELS); im = None
    mat_h = 0.205; mat_w = SQ * mat_h
    m1c = 0.735; m2c = m1c + mat_w + 0.014
    for M, ttl, cx, yl in [(C_data, "DESI data", m1c, True), (C_nf, "DESI-trained NF", m2c, False)]:
        axm = fig.add_axes(sqrect(cx, 0.200, mat_h))
        im = axm.imshow(M, cmap="RdBu_r", vmin=-1, vmax=1)
        axm.set_title(ttl, fontsize=14)
        axm.set_xticks(range(n)); axm.set_xticklabels(LABELS, rotation=45, ha="right", fontsize=9)
        axm.set_yticks(range(n)); axm.set_yticklabels(LABELS if yl else [""] * n, fontsize=9)
        for i in range(n):
            for j in range(n):
                axm.text(j, i, f"{M[i,j]:.2f}", ha="center", va="center", fontsize=5.8,
                         color="white" if abs(M[i, j]) > 0.6 else "black")
    cax = fig.add_axes([m2c + mat_w / 2 + 0.010, 0.200, 0.009, mat_h])
    cb = fig.colorbar(im, cax=cax); cb.set_label(r"corr$\,[\log(L_{\rm line}/L_{H\alpha})]$", fontsize=12)
    cb.ax.tick_params(labelsize=11)

    # ================= inter-panel arrows (fully in the white gaps) =================
    for xs, xe, ym, lbl, fs in [(0.330, 0.388, 0.500, r"$\mathbf{p(X\,|\,u)}$", 16),
                                (0.560, 0.618, 0.500, "sample &\nevaluate", 14)]:
        ...
        fig.text((xs + xe) / 2, ym + 0.030, lbl, ha="center", va="bottom",
                 fontsize=fs, color=ACCENT, fontweight="bold")
