# defend_selection_4cuts.py
#
# Intended selection, dropping ONLY the continuum-S/N cut:
#   (1) type == GALAXY  (DESI: main/bright + ZWARN==0)
#   (2) z > 0.05, ZWARN == 0
#   (3) per-line S/N > 5 on ALL 9 lines INCLUDING Halpha
#       (Hbeta, Hgamma, [NII]6584, [SII]6716/6717, [SII]6731, [OII]3726, [OII]3729,
#        [OIII]5007, Halpha)
# Continuum S/N (SNR_R / SN_MEDIAN) is shown for reference but NOT cut on.
...
C_INCL = "#CC79A7"; C_CONT = "#0072B2"; C_LINE = "#D55E00"; C_Z = "#999999"
...
def pack(rec, flux_cols, kind, z, zwarn, logm, cont, base_extra=True):
    ...
    min9 = snr9.min(1).astype(np.float32)                  # weakest of ALL 9 lines (incl Halpha)
    arg9 = snr9.argmin(1).astype(np.int8)
    x, y = bpt_xy(rec, flux_cols)
    base = np.isfinite(z) & (z > 0) & np.isfinite(zwarn) & (zwarn == 0) & base_extra
    return dict(min9=min9, arg9=arg9, x=x, y=y, z=z.astype(np.float32),
                logm=logm.astype(np.float32), cont=cont.astype(np.float32), base=base)
...
def masks(a):
    # FOUR cuts: z>0.05, ZWARN==0, GALAXY, per-line S/N>5 on all 9 lines. NO continuum-S/N cut.
    base = a["base"]
    pz = a["z"] > Z_MIN
    pl = a["min9"] > SNR_MIN
    sel = base & pz & pl
    ex_z = base & ~pz
    ex_l = base & pz & ~pl
    return dict(sel=sel, ex_z=ex_z, ex_l=ex_l, base=base)
...
def main():
    ...
    for tag, A in DATA.items():
        m = masks(A); M[tag] = m
        print(f"[{tag}] base={m['base'].sum():,}  selected(4 cuts, NO continuum)={m['sel'].sum():,}", flush=True)
        print(f"       excl z<0.05={m['ex_z'].sum():,}  excl per-line-S/N={m['ex_l'].sum():,}", flush=True)
        below = (A["cont"][m["sel"]] <= SNR_MIN)
        print(f"       of the {m['sel'].sum():,} selected, {int(below.sum()):,} ({below.mean():.0%}) "
              f"have continuum S/N<=5 (would have been removed by the old continuum cut)", flush=True)
        lim = A["arg9"][m["ex_l"]]; frac = np.bincount(lim, minlength=9) / max(len(lim), 1)
        order = np.argsort(frac)[::-1]
        print(f"       per-line-excluded limited by:",
              ", ".join(f"{LINE_LABELS[i]}={frac[i]:.0%}" for i in order[:4]), flush=True)

    # ---------- FIGURE 1: BPT colored by continuum S/N (ref), weakest-line S/N, redshift ----------
    ...
    for r, tag in enumerate(["SDSS", "DESI"]):
        ...
        c0 = fig.colorbar(h0, ax=ax[r, 0], shrink=0.85); c0.set_label(r"med. $\log_{10}$(continuum S/N)  [NOT cut]")
        c0.ax.axhline(np.log10(SNR_MIN), color="k", lw=2)
        h1 = bpt_col(ax[r, 1], A["x"][b], A["y"][b], np.log10(np.clip(A["min9"][b], 1e-2, None)), CMAP_SNR, nrm)
        contour(ax[r, 1], A["x"][m["sel"]], A["y"][m["sel"]])
        c1 = fig.colorbar(h1, ax=ax[r, 1], shrink=0.85); c1.set_label(r"med. $\log_{10}$(weakest-line S/N, all 9)")
    ...
    ax[0, 0].set_title("continuum S/N (reference, not cut)"); ax[0, 1].set_title("weakest-line S/N (all 9)")
    ax[0, 2].set_title("redshift")
    ax[0, 0].plot([], [], "k-", lw=1.7, label="selected 68/95%"); ax[0, 0].legend(loc="lower left")
    fig.suptitle("Intended selection (4 cuts incl. per-line S/N>5 on all 9 lines; NO continuum cut)", fontsize=16)
    fig.savefig("bpt_selection_4cuts.png", dpi=210, bbox_inches="tight")
    print("Saved: bpt_selection_4cuts.png", flush=True)

    # ---------- FIGURE 2: distributions (included, excl per-line, excl z) ----------
    ...
    for r, tag in enumerate(["SDSS", "DESI"]):
        ...
        ax0.hist(np.clip(A["cont"][m["ex_l"]], 0.3, 300), bins=sb, density=True, histtype="step", lw=2,
                 color=C_LINE, label="excl: per-line")
        ax0.axvline(SNR_MIN, color="k", ls=":", lw=1.6)
        ax0.text(SNR_MIN * 1.05, 0.5, "old continuum cut", rotation=90, va="center", fontsize=9,
                 transform=ax0.get_xaxis_transform())
        ax0.set_xscale("log"); ax0.set_xlabel("continuum S/N (not cut)")
        ax0.set_ylabel(f"{tag}\nnormalized density"); ax0.legend()
        ax1 = a2[r, 1]
        ax1.hist(np.clip(A["min9"][m["sel"]], 0.3, 300), bins=sb, density=True, histtype="step", lw=2,
                 color=C_INCL, label="included")
        ax1.hist(np.clip(A["min9"][m["ex_l"]], 0.3, 300), bins=sb, density=True, histtype="step", lw=2,
                 color=C_LINE, label=f"excl: per-line ({m['ex_l'].sum():,})")
        ax1.axvline(SNR_MIN, color="k", ls=":", lw=1.6); ax1.set_xscale("log")
        ax1.set_xlabel("weakest-line S/N (all 9)"); ax1.legend()
        ax2 = a2[r, 2]
        ...
        ax2.axvline(Z_MIN, color="k", ls=":", lw=1.6); ax2.set_xlabel("redshift"); ax2.legend()
        ax3 = a2[r, 3]; mb = np.linspace(7.5, 12.5, 60)
        for msk, col, lab in [(m["sel"], C_INCL, "included"), (m["ex_l"], C_LINE, "excl: line")]:
    ...
    a2[0, 0].set_title("continuum S/N (not cut)"); a2[0, 1].set_title("weakest-line S/N (all 9)")
    a2[0, 2].set_title("redshift"); a2[0, 3].set_title("stellar mass")
    fig2.suptitle("Intended selection (4 cuts, per-line S/N>5 on all 9 incl. Halpha; no continuum cut)",
                  fontsize=14)
    fig2.savefig("selection_excluded_4cuts.png", dpi=210, bbox_inches="tight")
    print("Saved: selection_excluded_4cuts.png", flush=True)
