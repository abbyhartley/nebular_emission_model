"""
Correlation matrices (companion to plot_covariance_matrices.py) of the 8 target
log line ratios log10(L_line/L_Hα), normalized to [-1, 1] with a unit diagonal so
the off-diagonal co-variation structure is visible (in covariance the [O III]
variance dominates the scale). Panels:
  (top-left) SDSS data   (top-right) SDSS-trained NF
  (bot-left) DESI data   (bot-right) DESI-trained NF
NF samples drawn at the native survey's (logM*, logL_Ha) conditioning.
"""
...
def data_log_ratios(df, meta, *, survey):
    ...
    return np.log10(F[good]) - np.log10(fha[good])[:, None]
...
def main():
    ...
    corr = {
        "sdss_data": np.corrcoef(data_log_ratios(df_sdss, meta_s, survey="sdss"), rowvar=False),
        "sdss_nf":   np.corrcoef(sample_log_ratios(flow_s, meta_s, df_sdss, seed=SEED + 10), rowvar=False),
        "desi_data": np.corrcoef(data_log_ratios(df_desi, meta_d, survey="desi"), rowvar=False),
        "desi_nf":   np.corrcoef(sample_log_ratios(flow_d, meta_d, df_desi, seed=SEED + 20), rowvar=False),
    }
    ...
    cbar.set_label(r"Correlation of $\log_{10}(L_{\rm line}/L_{\mathrm{H}\alpha})$", fontsize=13)
    fig.suptitle(r"Correlation of the 8 target log line ratios", fontsize=15)
    for ext in ("png", "pdf"):
        Path(REPO + "figs").mkdir(exist_ok=True)
        out = REPO + f"figs_ALTB/correlation_matrices.{ext}"
